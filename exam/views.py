...
class ExamDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for student in eligible_students:
            # Use filter() to get all submissions and order by most recent
            ...
            latest_completed_submission = submissions.filter(is_completed=True).first()

            # Check for an ongoing submission that is not yet completed
            ongoing_submission = submissions.filter(is_completed=False).first()

            if latest_completed_submission:
                status = 'Completed'
                score_display = f"{latest_completed_submission.score}/{latest_completed_submission.total_marks} ({latest_completed_submission.percentage:.1f}%)"
                submitted_at = latest_completed_submission.submitted_at
            elif ongoing_submission:
                status = 'In Progress'
...
class StartExamView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request, pk):
        ...
        # ➡️ Add this security check ⬅️
        if not exam.can_student_attempt(student):
            messages.error(request, 'You have no attempts remaining for this exam.')
