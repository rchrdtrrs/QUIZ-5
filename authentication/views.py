...
class StudentProfileView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Get all exam submissions for this student that are completed
        all_submissions = ExamSubmission.objects.filter(
            student=student,
            is_completed=True
        ).select_related('exam').order_by('-submitted_at')

        # --- ADDED CODE TO CALCULATE STATISTICS ---
        total_exams_taken = all_submissions.count()

        passed_exams_count = 0
        failed_exams_count = 0

        for submission in all_submissions:
            if submission.percentage >= submission.exam.passing_percentage:
                passed_exams_count += 1
            else:
                failed_exams_count += 1

        pass_rate = (passed_exams_count / total_exams_taken) * 100 if total_exams_taken > 0 else 0
        # --- END OF ADDED CODE ---
